...
export default function EditPostPage({ params }: PageProps) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      setVideoIframe(post.video_iframe || post.youtube_url || "") // Support legacy youtube_url
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Error al actualizar el artículo")
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-4 py-4">
            ...
            <h1 className="text-2xl font-bold text-green-700">Editar Artículo</h1>
          </div>
        </div>
      </header>
